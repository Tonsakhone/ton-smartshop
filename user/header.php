		...
		<div class="col-md-6 header-middle">
			<form action="index.php" method="get" id="form">
				...
				<div class="section_room ">
						<input type="search" id="s" value=""  style="width:210%; border-right: none;  " placeholder=" ຊື່ສິນຄ້າ............" name="search" class="fontlaos" required>
				</div>
				<div class="sear-sub ">
...
<?php 
	$sql_cart = "SELECT sum(total) FROM bookig WHERE user_name = '$user' AND user_pw = $pw";
	$rs_cart = mysqli_query($con,$sql_cart);
	$row_cart = mysqli_fetch_array($rs_cart);
	$price_cart = $row_cart['sum(total)'];

	$sql_item="SELECT count(no)  FROM bookig WHERE user_name='$user' AND user_pw=$pw";
	$rs_item = mysqli_query($con,$sql_item);
	$row_item = mysqli_fetch_array($rs_item);
	$item = $row_item['count(no)'];
			...
			<div class="cart box_1">
						<a href="basket.php">
							<h3> <div class="total fontlaos">
								...
								<span class=""><?php if ($price_cart == NULL){
									echo '0';
								}else{
									echo number_format($price_cart);
								} ?> kip
									
								</span><br> (<span id="" class="">
									<?php 
									if ($item == NULL) {
										echo '0';
									}else{
